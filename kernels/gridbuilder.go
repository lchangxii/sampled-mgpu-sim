...
//var collectMemFootPrintFlag = flag.Bool("collectMemFoot", false, "Collect Memory Foot Data.")
...
func (b *gridBuilderImpl) YDim() int {
    return b.ydim;
}
func (b *gridBuilderImpl) ZDim() int {
    return b.zdim;
}

func (b *gridBuilderImpl) XDim() int {
    return b.xdim;
}
...
func (b *gridBuilderImpl) NextWG() *WorkGroup {
	wg := NewWorkGroup()
//    wg.CollectWfMem = *collectMemFootPrintFlag
	for {
...
func (b *gridBuilderImpl) formWavefronts(wg *WorkGroup) {
	...
	for i, wi := range wg.WorkItems {
		...
		if inWGID%wavefrontSize == 0 {
			...
			wf.WG = wg
//            wf.CollectWfMem = *collectMemFootPrintFlag
