...
type PageTable interface {
	...
	Find(pid vm.PID, Addr uint64) (Page, bool)
	Update(page Page)
}
...
func NewPageTable(log2PageSize uint64) PageTable {
	return &pageTableImpl{
		...
		tables:       make(map[vm.PID]*processTable),
	}
}
...
type pageTableImpl struct {
	sync.Mutex
	log2PageSize uint64
	tables       map[vm.PID]*processTable
}

func (pt *pageTableImpl) getTable(pid vm.PID) *processTable {
	pt.Lock()
	defer pt.Unlock()
...
// address.
func (pt *pageTableImpl) Remove(pid vm.PID, vAddr uint64) {
...
func (pt *pageTableImpl) Find(pid vm.PID, vAddr uint64) (Page, bool) {
	table := pt.getTable(pid)
