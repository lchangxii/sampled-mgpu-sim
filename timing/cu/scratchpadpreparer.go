...
type ScratchpadPreparer interface {
	Prepare(instEmuState utils.InstEmuState, wf *wavefront.Wavefront)
	Commit(instEmuState utils.InstEmuState, wf *wavefront.Wavefront)
}
...
func (p *ScratchpadPreparerImpl) Prepare(
	instEmuState utils.InstEmuState,
...
func (p *ScratchpadPreparerImpl) prepareFlat(
	instEmuState utils.InstEmuState, wf *wavefront.Wavefront,
) {
	inst := instEmuState.Inst()
	sp := instEmuState.Scratchpad()
	layout := sp.AsFlat()
