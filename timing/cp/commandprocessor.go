...
type CommandProcessor struct {
	*sim.TickingComponent
    ...
    cusForSampling  []CUInterfaceForCP
	DMAEngine          sim.Port
	Driver             sim.Port
	TLBs               []sim.Port
	CUs                []sim.Port
...
func (p *CommandProcessor) RegisterCU(cu CUInterfaceForCP) {
    ...
    p.cusForSampling = append(p.cusForSampling,cu)
...
func (p *CommandProcessor) resetForSampling( )  {
	...
	for _, d := range p.cusForSampling {
        d.ResetForSampling()
    }
}
...
func (p *CommandProcessor) processLaunchKernelReq(
    ...
    if *sampledrunner.BranchSampledFlag {
        sampledrunner.Branchsampledengine.Reset()
    }
    if *sampledrunner.SampledRunnerFlag {
        sampledrunner.Sampledengine.Reset()
    }

    if *sampledrunner.KernelSampledFlag  {
        sampledrunner.Kernelsampledengine.Reset(false)
    }
    if *profiler.WfProfilingFlag{
        profiler.Wffinalfeature.Reset()
    }
	d.StartDispatching(req,now)
