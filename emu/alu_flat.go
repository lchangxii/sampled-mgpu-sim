...
//nolint:gocyclo
//nolint:funlen
func (u *ALUImpl) runFlat(state utils.InstEmuState) {
	inst := state.Inst()
	switch inst.Opcode {
	case 16:
...
func (u *ALUImpl) runFlatLoadUByte(state utils.InstEmuState) {
...
func (u *ALUImpl) runFlatLoadUShort(state utils.InstEmuState) {
	sp := state.Scratchpad().AsFlat()
	pid := state.PID()
	for i := uint(0); i < 64; i++ {
		if !laneMasked(sp.EXEC, i) {
...
func (u *ALUImpl) runFlatLoadDWord(state utils.InstEmuState) {
	sp := state.Scratchpad().AsFlat()
	pid := state.PID()
	for i := uint(0); i < 64; i++ {
		if !laneMasked(sp.EXEC, i) {
...
func (u *ALUImpl) runFlatLoadDWordX2(state utils.InstEmuState) {
	sp := state.Scratchpad().AsFlat()
	pid := state.PID()
	for i := uint(0); i < 64; i++ {
		if !laneMasked(sp.EXEC, i) {
...
func (u *ALUImpl) runFlatLoadDWordX4(state utils.InstEmuState) {
	sp := state.Scratchpad().AsFlat()
	pid := state.PID()
	for i := uint(0); i < 64; i++ {
		if !laneMasked(sp.EXEC, i) {
...
func (u *ALUImpl) runFlatStoreDWord(state utils.InstEmuState) {
	sp := state.Scratchpad().AsFlat()
	pid := state.PID()
...
func (u *ALUImpl) runFlatStoreDWordX2(state utils.InstEmuState) {
	sp := state.Scratchpad().AsFlat()
	pid := state.PID()
	for i := uint(0); i < 64; i++ {
		if !laneMasked(sp.EXEC, i) {
...
func (u *ALUImpl) runFlatStoreDWordX3(state utils.InstEmuState) {
	sp := state.Scratchpad().AsFlat()
	pid := state.PID()
	for i := uint(0); i < 64; i++ {
		if !laneMasked(sp.EXEC, i) {
...
func (u *ALUImpl) runFlatStoreDWordX4(state utils.InstEmuState) {
	sp := state.Scratchpad().AsFlat()
	pid := state.PID()
	for i := uint(0); i < 64; i++ {
		if !laneMasked(sp.EXEC, i) {
