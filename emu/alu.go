...
//nolint:gocyclo
func (u *ALUImpl) Run(state utils.InstEmuState) {
	inst := state.Inst()
	// fmt.Printf("%s\n", inst.String(nil))
...
func (u *ALUImpl) runSMEM(state utils.InstEmuState) {
	inst := state.Inst()
	switch inst.Opcode {
	case 0:
...
func (u *ALUImpl) runSLOADDWORD(state utils.InstEmuState) {
...
func (u *ALUImpl) runSLOADDWORDX2(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSMEM()
...
func (u *ALUImpl) runSLOADDWORDX4(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSMEM()
	spRaw := state.Scratchpad()
	pid := state.PID()
...
func (u *ALUImpl) runSLOADDWORDX8(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSMEM()
	spRaw := state.Scratchpad()
	pid := state.PID()
...
//nolint:gocyclo
func (u *ALUImpl) runSOPP(state utils.InstEmuState) {
	inst := state.Inst()
	switch inst.Opcode {
	case 0: // S_NOP
	// Do nothing
...
func (u *ALUImpl) runSCBRANCH(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPP()
	imm := asInt16(uint16(sp.IMM & 0xffff))
	sp.PC = uint64(int64(sp.PC) + int64(imm)*4)
}

func (u *ALUImpl) runSCBRANCHSCC0(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPP()
	imm := asInt16(uint16(sp.IMM & 0xffff))
	if sp.SCC == 0 {
		sp.PC = uint64(int64(sp.PC) + int64(imm)*4)
...
func (u *ALUImpl) runSCBRANCHSCC1(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPP()
	imm := asInt16(uint16(sp.IMM & 0xffff))
	if sp.SCC == 1 {
		sp.PC = uint64(int64(sp.PC) + int64(imm)*4)
...
func (u *ALUImpl) runSCBRANCHVCCZ(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPP()
	imm := asInt16(uint16(sp.IMM & 0xffff))
	if sp.VCC == 0 {
		sp.PC = uint64(int64(sp.PC) + int64(imm)*4)
...
func (u *ALUImpl) runSCBRANCHVCCNZ(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPP()
	imm := asInt16(uint16(sp.IMM & 0xffff))
	if sp.VCC != 0 {
		sp.PC = uint64(int64(sp.PC) + int64(imm)*4)
...
func (u *ALUImpl) runSCBRANCHEXECZ(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPP()
	imm := asInt16(uint16(sp.IMM & 0xffff))
	if sp.EXEC == 0 {
		sp.PC = uint64(int64(sp.PC) + int64(imm)*4)
...
func (u *ALUImpl) runSCBRANCHEXECNZ(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPP()
	imm := asInt16(uint16(sp.IMM & 0xffff))
	if sp.EXEC != 0 {
		sp.PC = uint64(int64(sp.PC) + int64(imm)*4)
...
func (u *ALUImpl) dumpScratchpadAsSop2(state utils.InstEmuState, byteCount int) string {
	scratchpad := state.Scratchpad()
	layout := new(utils.SOP2Layout)
