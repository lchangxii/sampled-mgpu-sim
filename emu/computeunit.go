...
func (cu *ComputeUnit) runWG(
    ...
    if *profiler.ProfileApplication {
        wgfeature := &profiler.WorkGroupFeature {
            IDX : wg.IDX,
            IDY : wg.IDY,
            IDZ : wg.IDZ,
        }
    	for _, wf := range cu.wfs[wg] {
             wgfeature.AddWavefrontFeature(wf.Wf_feature)
        }
        profiler.WorkGroup_tensor.AddWorkGroupFeature(wgfeature, wg.IDX,wg.IDY,wg.IDZ)
    }
...
func (cu *ComputeUnit) logInst(wf *Wavefront, inst *insts.Inst) {
	...
	cu.InvokeHook(ctx)
    profiler.Inst_profiler.Collect()
}
