...
func (u *ALUImpl) runSOPK(state utils.InstEmuState) {
	inst := state.Inst()
	switch inst.Opcode {
	case 0:
		u.runSMOVKI32(state)
...
func (u *ALUImpl) runSMOVKI32(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPK()
	imm := asInt16(uint16(sp.IMM & 0xffff))
	sp.DST = uint64(imm)
}

func (u *ALUImpl) runSCMPKLGI32(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPK()
	imm := asInt16(uint16(sp.IMM & 0xffff))
...
func (u *ALUImpl) runSMULKI32(state utils.InstEmuState) {
	sp := state.Scratchpad().AsSOPK()
	imm := asInt16(uint16(sp.IMM & 0xffff))
